...
Upon tests on the Million Song dataset with 10<sup>5</sup> lines (or a generated distance matrix of size 10<sup>10</sup>) of floating point data, Helios achieved more than hundred times the performance of the sequential version (without any form of parallelism) with a tiled (500 x 500) distance matrix computation handled by 64 processes each tied to a separate CPU core, with eight threads per core. (with multithreading within a process, all threads can share the dataset and re-use it from cache) 

<br>
<br>

Note that this has been tested on a compute cluster. A caveat here would be to not run this computation on a typical laptop, given that it requires around 75 GiB of main memory to store the dataset. (10<sup>5</sup>\*10<sup>5</sup>\*8/1024/1024 = 76k MiB or ~74.5 GiB) is required to store the dataset in memory. Another caveat here would be to avoid use of excessive threads, since the use of more threads (or even processes, as the parallel efficiency decreases after a certain threshold and more datasets have to be created for increasing number of process ranks) is not always beneficial. This is an empirically proven statement based on my experimental observations, given that I tried with the maximum number possible, inclusive of hardware threads (i.e., with hyper-threading).
